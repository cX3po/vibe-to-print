...
template_library.py
-------------------
Library of parametric OpenSCAD templates for the Manual (No AI) mode.
...
Each template is a dict with:
  ...
  dims:          list of {id, question, default, unit}
  scad_modules:  OpenSCAD module definitions (uses dim IDs as global variable names)
  scad_call:     final OpenSCAD call (wrapped in a translate by generate_scad)

Public API
----------
TEMPLATES               list of all template dicts
CATEGORIES              sorted list of unique category names
search(query, cat)      -> filtered list of templates
get(template_id)        -> template dict or None
generate_scad(t, dim_values, bed_cx, bed_cy) -> str  (complete .scad file)
"""
...
module knob_body() {
    // Main cylinder with raised grip bumps
    union() {
...
module knob() {
    ...
    # ── 2. Round Knob — Round Shaft with Set-Screw ────────────────────────────
    {
        "id":          "knob_round_shaft",
...
module set_screw_channel() {
    // Horizontal set-screw hole, positioned 8 mm up from bottom
    translate([0, 0, 8])
        rotate([90, 0, 0])
...
module knob() {
    ...
    # ── 3. Pointer Knob (D-shaft with indicator line) ─────────────────────────
    {
        "id":          "knob_pointer",
...
module pointer_line() {
    // Raised indicator ridge on top face, pointing at 12 o'clock
...
module knob() {
    ...
    # ── 4. Simple Rectangular Box (Open Top) ─────────────────────────────────
    {
        "id":          "box_open",
...
module rounded_box(w, d, h, r) {
    // Rounded-corner box using hull of four cylinders
    hull() {
...
module box() {
    ...
    outer_h = inner_h + wall;  // open top — no top wall
    difference() {
    ...
    # ── 5. Box with Press-Fit Lid ─────────────────────────────────────────────
    {
        ...
        "description": "Rectangular box with a matching press-fit lid. Prints as two separate parts.",
        "dims": [
            {"id": "inner_w",    "question": "Inner width (mm)",           "default": 80,  "unit": "mm"},
            {"id": "inner_d",    "question": "Inner depth (mm)",           "default": 60,  "unit": "mm"},
...
module lid() {
    ...
    union() {
        ...
        // Inner rim that presses into the box
...
module both_parts() {
    // Box on the left, lid to the right with a gap
    box_body();
    translate([inner_w + 2 * wall + 10, 0, 0])
        lid();
}""",
    ...
    # ── 6. End Cap / Tube Plug ────────────────────────────────────────────────
    {
        ...
        "tags":        ["cap", "plug", "end cap", "tube", "pipe", "cover",
                        "stopper", "fitting"],
        "description": "Cylindrical plug that inserts into the end of a tube or pipe, with a retention flange.",
        "dims": [
            {"id": "plug_d",    "question": "Plug outer diameter — fits INSIDE tube (mm)", "default": 24.0, "unit": "mm"},
            {"id": "plug_h",    "question": "Plug insertion depth (mm)",                    "default": 15,   "unit": "mm"},
            {"id": "flange_d",  "question": "Flange outer diameter — sits outside tube (mm)", "default": 29, "unit": "mm"},
            {"id": "flange_h",  "question": "Flange thickness (mm)",                        "default": 3,   "unit": "mm"},
            {"id": "bore_d",    "question": "Centre bore diameter (0 for solid plug) (mm)", "default": 0,   "unit": "mm"},
        ],
        "scad_modules": """\
module end_cap() {
    difference() {
    ...
    # ── 7. L-Bracket ─────────────────────────────────────────────────────────
    {
        ...
        "description": "Right-angle mounting bracket with a specified hole pattern.",
        "dims": [
            {"id": "width",       "question": "Bracket width (into the page) (mm)", "default": 30,  "unit": "mm"},
...
module l_bracket() {
    difference() {
        union() {
            // Vertical arm
            arm(arm1_len);
            // Horizontal arm (rotated 90 deg, offset so corner meets flush)
            translate([0, 0, 0])
                rotate([0, -90, 0])
                    translate([-arm2_len, 0, 0])
                        arm(arm2_len);
        }
    ...
    # ── 8. Cable Clip ─────────────────────────────────────────────────────────
    {
        ...
        "dims": [
            {"id": "cable_d",  "question": "Cable / wire outer diameter (mm)",      "default": 6,   "unit": "mm"},
...
module cable_clip() {
    ...
    difference() {
        union() {
            // Mounting base
            translate([-r, -r, 0])
                cube([r * 2, r + wall_t, base_h]);
            // Clip ring — arc from gap to gap
            translate([0, 0, base_h])
                difference() {
                    cylinder(h = clip_w, r = r, $fn = 64);
                    // Inner bore
                    translate([0, 0, -0.5])
                        cylinder(h = clip_w + 1, r = cable_d / 2, $fn = 64);
                    // Opening gap (front)
                    translate([-gap_mm/2, -(r + 1), -0.5])
                        cube([gap_mm, r + 2, clip_w + 1]);
                }
        }
        // Mounting screw hole through base
        translate([0, -r/2, -0.5])
            cylinder(h = base_h + 1, d = screw_d, $fn = 32);
    }
}""",
...
module spacer() {
    difference() {
        ...
        translate([0, 0, -0.5])
            cylinder(h = height + 1, d = inner_d, $fn = 64);
    }
}""",
        "scad_call": "spacer()",
    },
    ...
    # ── 11. Drawer Pull / Handle ──────────────────────────────────────────────
    {
        ...
        "scad_modules": """\
module end_post() {
    // Vertical post that lifts the bar off the surface
    cylinder(h = grip_h, d = grip_w, $fn = 48);
}

module grip_bar() {
    // Horizontal bar between the two posts
    hull() {
        translate([0, 0, grip_h - bar_d/2])
            rotate([0, 90, 0])
                cylinder(h = grip_len, d = bar_d, $fn = 48);
    }
}
...
module drawer_pull() {
    ...
    difference() {
        ...
        translate([offset_x, 0, -0.5])
            cylinder(h = grip_h + 1, d = screw_d, $fn = 32);
        translate([offset_x + span, 0, -0.5])
            cylinder(h = grip_h + 1, d = screw_d, $fn = 32);
    }
}""",
    ...
    # ── 12. Button / Switch Cap ───────────────────────────────────────────────
    {
        ...
        "dims": [
            ...
            {"id": "stem_gap", "question": "Socket clearance/gap (mm, ~0.2–0.4)",    "default": 0.2, "unit": "mm"},
        ],
        "scad_modules": """\
module button_cap() {
    ...
    difference() {
        union() {
            ...
            difference() {
                ...
                translate([0, 0, -0.5])
                    cylinder(h = skirt_h + 1, d = cap_d - 2, $fn = 64);
            }
            // Main dome cap
            translate([0, 0, skirt_h])
                cylinder(h = cap_h, d = cap_d, $fn = 64);
        }
        // Stem socket (square, from bottom)
        translate([-socket_w/2, -socket_w/2, -0.5])
            cube([socket_w, socket_w, stem_h + 0.5]);
    }
}""",
...
CATEGORIES: list[str] = sorted({t["category"] for t in TEMPLATES})

_BY_ID: dict[str, dict] = {t["id"]: t for t in TEMPLATES}


def get(template_id: str) -> dict | None:
    """Return a template dict by ID, or None."""
    return _BY_ID.get(template_id)


def search(query: str = "", category: str = "All") -> list[dict]:
    """
    Filter TEMPLATES by category and/or keyword query.
    Returns matching templates ordered by relevance (exact name match first).
    """
    q = query.strip().lower()
    results = []
    for t in TEMPLATES:
        if category not in ("All", "") and t["category"] != category:
            continue
        if not q:
            results.append((0, t))
            continue
...
def _parse_num(raw: str, default: float) -> str:
    """Extract the numeric part from a user value like '12.5 mm', return as string."""
    raw = raw.strip()
    if not raw:
        return str(default)
    part = raw.split()[0]
...
def generate_scad(template: dict, dim_values: dict[str, str],
                  bed_cx: float, bed_cy: float) -> str:
    """
    Build a complete .scad file from a template + user-supplied dimension values.
    dim_values: {dim_id: "value [unit]"}  — unit part is stripped automatically.
    bed_cx, bed_cy: bed centre coordinates for the translate wrapper.
    """
    lines = [
        "// ─────────────────────────────────────────────────────────────",
        "// Vibe-to-Print Engine — Template Mode (No AI)",
        f"// Template : {template['name']}",
        f"// Category : {template['category']}",
        "// ─────────────────────────────────────────────────────────────",
        "",
        "// ── Dimensions (edit these to resize the model) ──────────────",
    ]

    for dim in template["dims"]:
        raw     = dim_values.get(dim["id"], "")
        num_str = _parse_num(raw, dim["default"])
        unit    = f" // {dim['unit']}" if dim["unit"] else ""
        lines.append(f"{dim['id']} = {num_str};{unit}  // {dim['question']}")
